...
class SimpleNet(torch.nn.Module):
    ...
    def load(
        ...
        device,
        input_shape,
        pretrain_embed_dimension, # 1536 which is number of features of WRN50 (Layer 2 & 3)
        ...
        **kwargs,
        ...
        # feature aggregator that consists of backbone and information regariding the feature extraction
        self.backbone = Backbone(model_id=backbone_id, layers_needed=layers_to_extract_from, layer_cut=True)
        #backbone.to(device)
        ...
        feature_aggregator = self.backbone.to(self.device)
        # feature_dimensions = feature_aggregator(torch.rainput_shape) # just determines the length of the feature vector
        feature_dimensions = feature_aggregator.feature_dim
        
        self.forward_modules["feature_aggregator"] = feature_aggregator # 1st module that was added to forward modules

        preprocessing = common.Preprocessing(
            feature_dimensions, pretrain_embed_dimension
        )
        self.forward_modules["preprocessing"] = preprocessing # 2nd module that was added to forward modules
        ...
        self.forward_modules["preadapt_aggregator"] = preadapt_aggregator # 3rd module in forward modules
    ...
    def test(self, training_data, test_data): # not working!
        ...
        print('Path is: ' , ckpt_path)
        if os.path.exists(ckpt_path):
            print('Loading model from: ', ckpt_path)
            state_dicts = torch.load(ckpt_path, map_location=self.device)
            if "pretrained_enc" in state_dicts:
                self.feature_enc.load_state_dict(state_dicts["pretrained_enc"])# there are no encoders neither decoders ???
