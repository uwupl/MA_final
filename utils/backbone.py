...
class Backbone(nn.Module):
    def __init__(
        ...
        self.need_for_own_last_layer = sigmoid_in_last_layer or exclude_relu or prune_output_layer[0] or need_for_own_last_layer or lrelu_in_last_layer or softmin_in_last_layer or quantize_qint8_prepared
    ...
    def procedure_resnet(self):
        ...
        if self.layer_cut and not self.need_for_own_last_layer:
            ...
            if self.prune_l1_unstructured[0]:
                ...
                self.model = prune_model_l1_strucured(self.model, pruning_perc=self.prune_l1_unstructured[1])
                print('done')
            aux_hook = (self.softmin_in_last_layer or self.lrelu_in_last_layer) and self.quantize_qint8_prepared
            if self.hooks_needed:
                if len(self.layers_needed) > 1:
                    for layer in self.layers_needed[:-1]:
                ...
                self.model[-1].register_forward_hook(self.hook_t if not aux_hook else self.hook_ResNet) # last layer
